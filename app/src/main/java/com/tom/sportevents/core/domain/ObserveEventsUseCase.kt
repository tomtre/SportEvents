...
class ObserveEventsUseCase @Inject constructor(
    ...
    override fun invoke(): Flow<Result<List<FormattedEventItem>>> {
        ...
        return flow { emit(eventsRepository.getEvents()) }
            ...
            .combine(timeModificationEventSource.timeModificationEvent.onStart { emit(Unit) }) { result, _ -> result }
            ...
            .transformAndEmitLatestPeriodically(REFRESH_MODEL_INTERVAL_MILLI) { result ->
                when (result) {
                    is Result.Success -> {
                        ...
                        val formattedList = sortedList.map { it.toFormattedEventItem(dateFormatter) }
    ...
    private fun EventItem.toFormattedEventItem(dateFormatter: DateFormatter): FormattedEventItem =
        FormattedEventItem(
            id = id,
            title = title,
            subtitle = subtitle,
